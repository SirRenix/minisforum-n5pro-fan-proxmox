...
if dpkg-query -W -f='${Status}' "$PKG-dkms" 2>/dev/null | grep -q "install ok installed"; then
    echo "Treiber ist als Paket $PKG-dkms installiert: dieses Skript (n5-fand + Handinstallation 0.2.0) nicht verwenden."
    echo "n5-fand allein: systemctl disable --now n5-fand; Dateien siehe unten im Skript."
    exit 1
fi
